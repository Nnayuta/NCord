...
async function setupProcessAudioTrack() {
  ...
  try {
    ...
    // Criar nó master de ganho do mixer
    mixerMasterGainNode = processAudioContext.createGain();
    mixerMasterGainNode.gain.value = mixerMasterMuted ? 0 : mixerMasterVolume;
    mixerMasterGainNode.connect(processAudioDestination);
    ...
    processAudioWorkletNode.connect(mixerMasterGainNode);
    ...
    if (audioTracks && audioTracks.length > 0) {
      console.log('🎧 [LoveChat] Faixa de áudio exclusivo de janela criada com sucesso via AudioWorklet e roteada no Mixer!');
...
function cleanupProcessAudio() {
  ...
  if (typeof stopAllMixerAppsNodes === 'function') {
    stopAllMixerAppsNodes();
  }
  ...
  if (mixerMasterGainNode) {
    try { mixerMasterGainNode.disconnect(); } catch (e) {}
    mixerMasterGainNode = null;
  }
...
// ==========================================
// GERENCIADOR DO MIXER DE ÁUDIO DE TRANSMISSÃO
// ==========================================
let isMixerOpen = false;
let mixerMasterVolume = 1.0;
let mixerMasterMuted = false;
let mixerAntiEcho = true;
let mixerMasterGainNode = null;
const mixerAppsState = new Map(); // pid -> { pid, name, appIcon, enabled, volume, muted, gainNode, workletNode }

async function startMixerAppNode(app) {
  if (!processAudioContext || !mixerMasterGainNode) return;
  try {
    if (!app.workletNode) {
      app.workletNode = new AudioWorkletNode(processAudioContext, 'window-audio-processor', {
        numberOfInputs: 0,
        numberOfOutputs: 1,
        outputChannelCount: [2]
      });
      app.gainNode = processAudioContext.createGain();
      const currentGain = app.muted ? 0 : app.volume;
      app.gainNode.gain.value = currentGain;
      app.workletNode.connect(app.gainNode);
      app.gainNode.connect(mixerMasterGainNode);
    }

    if (window.electronAPI && window.electronAPI.startMixerProcessAudio) {
      await window.electronAPI.startMixerProcessAudio(app.pid, true);
    }
  } catch (e) {
    console.error(`Erro ao iniciar nó de mixer para PID ${app.pid}:`, e);
  }
}

function stopMixerAppNode(app) {
  try {
    if (window.electronAPI && window.electronAPI.stopMixerProcessAudio) {
      window.electronAPI.stopMixerProcessAudio(app.pid);
    }
    if (app.workletNode) {
      app.workletNode.port.postMessage({ type: 'clear' });
...
function stopAllMixerAppsNodes() {
  for (const app of mixerAppsState.values()) {
    stopMixerAppNode(app);
  }
}

function setupAudioMixerUI() {
  const popover = document.getElementById('audio-mixer-popover');
  if (!popover) return;

  const btnMixerBottom = document.getElementById('btn-audio-mixer');
  const btnMixerStream = document.getElementById('btn-stream-mixer');
  const btnClose = document.getElementById('btn-close-mixer');
  const btnRefresh = document.getElementById('btn-refresh-mixer');
  const chkAntiEcho = document.getElementById('chk-mixer-anti-echo');
  const sliderMaster = document.getElementById('slider-mixer-master');
  const btnMasterMute = document.getElementById('btn-mixer-master-mute');
  const lblMasterVal = document.getElementById('lbl-mixer-master-val');
  const appsList = document.getElementById('mixer-apps-list');

  function updateMasterUI() {
    if (sliderMaster) sliderMaster.value = mixerMasterVolume;
    if (lblMasterVal) lblMasterVal.textContent = mixerMasterMuted ? 'Mudo' : `${Math.round(mixerMasterVolume * 100)}%`;
    if (btnMasterMute) {
      if (mixerMasterMuted) {
  ...
  function togglePopover(show) {
    const shouldShow = show !== undefined ? show : popover.classList.contains('hide');
    if (shouldShow) {
      popover.classList.remove('hide');
      if (btnMixerBottom) btnMixerBottom.classList.add('active');
      if (btnMixerStream) btnMixerStream.classList.add('active');
      loadMixerSources();
    } else {
      popover.classList.add('hide');
      if (btnMixerBottom) btnMixerBottom.classList.remove('active');
      if (btnMixerStream) btnMixerStream.classList.remove('active');
    }
    refreshIcons();
  }

  if (btnMixerBottom) btnMixerBottom.addEventListener('click', (e) => { e.stopPropagation(); togglePopover(); });
  if (btnMixerStream) btnMixerStream.addEventListener('click', (e) => { e.stopPropagation(); togglePopover(); });
  if (btnClose) btnClose.addEventListener('click', () => togglePopover(false));

  // Fechar ao clicar fora
  document.addEventListener('click', (e) => {
    if (!popover.classList.contains('hide')) {
      if (!popover.contains(e.target) && (!btnMixerBottom || !btnMixerBottom.contains(e.target)) && (!btnMixerStream || !btnMixerStream.contains(e.target))) {
        togglePopover(false);
      }
    }
  });
  ...
  if (chkAntiEcho) {
    chkAntiEcho.checked = mixerAntiEcho;
    chkAntiEcho.addEventListener('change', () => {
      mixerAntiEcho = chkAntiEcho.checked;
      showToast(mixerAntiEcho ? 'Anti-Eco ativado: Chamada silenciada na live 🛡️' : 'Anti-Eco desativado', 'info');
      renderMixerApps();
    });
  }
  ...
  if (sliderMaster) {
    sliderMaster.addEventListener('input', (e) => {
      mixerMasterVolume = parseFloat(e.target.value) || 0;
  ...
  async function loadMixerSources() {
    if (!window.electronAPI || !window.electronAPI.getAudioMixerSources) {
      if (appsList) {
        appsList.innerHTML = `<div class="mixer-loading">Mixer avançado disponível no aplicativo desktop LoveChat.</div>`;
      }
      return;
    }

    if (appsList) {
      appsList.innerHTML = `<div class="mixer-loading"><i data-lucide="loader-2" class="spin"></i> Buscando programas abertos...</div>`;
      refreshIcons();
    }
    ...
    try {
      ...
      if (res && res.success && res.apps) {
        // Atualizar mapa de estados mantendo volumes/mudos existentes
        res.apps.forEach((src) => {
          if (!mixerAppsState.has(src.pid)) {
            const isCallApp = src.isLoveChat;
            mixerAppsState.set(src.pid, {
              pid: src.pid,
              name: src.name || 'Aplicativo',
              appIcon: src.appIcon,
              thumbnail: src.thumbnail,
              isLoveChat: isCallApp,
              enabled: !isCallApp,
              volume: 1.0,
              muted: isCallApp,
              gainNode: null,
            ...
            const existing = mixerAppsState.get(src.pid);
            existing.name = src.name;
            existing.appIcon = src.appIcon;
          }
        });
        renderMixerApps();
      } else {
        if (appsList) {
          appsList.innerHTML = `<div class="mixer-loading">Nenhum aplicativo com janela encontrado.</div>`;
        }
      }
    } catch (err) {
      console.error('Erro ao carregar fontes do mixer:', err);
      if (appsList) {
        appsList.innerHTML = `<div class="mixer-loading">Erro ao carregar programas: ${err.message}</div>`;
      }
    }
  }
  ...
  function renderMixerApps() {
    ...
    if (mixerAppsState.size === 0) {
      appsList.innerHTML = `<div class="mixer-loading">Nenhum aplicativo detectado no momento.</div>`;
      return;
    }

    mixerAppsState.forEach((app) => {
      const item = document.createElement('div');
      item.className = `mixer-app-item ${app.enabled ? '' : 'inactive'}`;

      const iconHtml = app.appIcon
        ? `<img src="${app.appIcon}" class="mixer-app-icon" alt="Icon">`
        : `<i data-lucide="app-window" style="width:18px;height:18px;color:#c084fc;"></i>`;

      let badgeHtml = '';
      if (app.isLoveChat) {
        badgeHtml = mixerAntiEcho
          ? `<span class="mixer-app-badge disabled"><i data-lucide="shield" style="width:10px;height:10px;"></i> Bloqueado (Anti-Eco)</span>`
          : `<span class="mixer-app-badge"><i data-lucide="phone" style="width:10px;height:10px;"></i> Chamada</span>`;
      } else if (app.enabled) {
        badgeHtml = `<span class="mixer-app-badge"><i data-lucide="volume-2" style="width:10px;height:10px;"></i> Na Live</span>`;
      } else {
        badgeHtml = `<span class="mixer-app-badge disabled"><i data-lucide="volume-x" style="width:10px;height:10px;"></i> Fora da Live</span>`;
      }

      const percent = Math.round(app.volume * 100);

      item.innerHTML = `
        <div class="mixer-app-header">
          <div class="mixer-app-identity">
            ${iconHtml}
            <span class="mixer-app-name" title="${app.name} (PID: ${app.pid})">${app.name}</span>
          </div>
          ${badgeHtml}
        </div>
        <div class="mixer-slider-row">
          <button class="btn-mixer-mute ${app.muted || (app.isLoveChat && mixerAntiEcho) ? 'muted' : ''}" title="${app.muted ? 'Desmutar' : 'Mutar'}" ${app.isLoveChat && mixerAntiEcho ? 'disabled' : ''}>
            <i data-lucide="${app.muted || (app.isLoveChat && mixerAntiEcho) ? 'volume-x' : 'volume-2'}"></i>
          </button>
          <input type="range" min="0" max="1.5" step="0.05" value="${app.volume}" class="mixer-slider" ${app.isLoveChat && mixerAntiEcho ? 'disabled' : ''}>
          <span class="mixer-vol-val">${app.muted || (app.isLoveChat && mixerAntiEcho) ? '0%' : percent + '%'}</span>
        </div>
      `;

      const btnMute = item.querySelector('.btn-mixer-mute');
      const slider = item.querySelector('input[type="range"]');
      const valLabel = item.querySelector('.mixer-vol-val');

      if (btnMute && !(app.isLoveChat && mixerAntiEcho)) {
      ...
      if (slider && !(app.isLoveChat && mixerAntiEcho)) {
        slider.addEventListener('input', (e) => {
          app.volume = parseFloat(e.target.value) || 0;
          app.muted = false;
          if (app.gainNode) {
            app.gainNode.gain.value = app.volume;
          }
          if (valLabel) valLabel.textContent = `${Math.round(app.volume * 100)}%`;
        });
      }
  ...
  // Ouvir pacotes de áudio dos processos adicionais
  if (window.electronAPI && window.electronAPI.onMixerProcessAudioChunk) {
    window.electronAPI.onMixerProcessAudioChunk(({ pid, chunk }) => {
      const app = mixerAppsState.get(pid);
      if (app && app.workletNode && app.workletNode.port && chunk) {
        const uint8 = new Uint8Array(chunk);
        if (uint8.byteLength > 0) {
          const bufferCopy = uint8.buffer.slice(uint8.byteOffset, uint8.byteOffset + uint8.byteLength);
          app.workletNode.port.postMessage({ type: 'pcm', buffer: bufferCopy }, [bufferCopy]);
        }
      }
    });
  }
